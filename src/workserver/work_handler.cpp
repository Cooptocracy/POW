...
std::atomic<unsigned> coop_pow_server::job::job_id_dispenser{ 1 };

coop_pow_server::job::job ()
{
	job_id = job_id_dispenser.fetch_add (1);
}

/** Currently we support work on a single device, hence a single-thread pool */
coop_pow_server::work_handler::work_handler (coop_pow_server::config const & config_a, std::shared_ptr<spdlog::logger> const & logger_a)
    : config (config_a)
    , logger (logger_a)
    , pool (config.devices.size ())
{
	...
	{
		std::shared_ptr<coop_pow::driver> driver;
		if (device.type == coop_pow_server::config::device::device_type::cpu)
		{
			driver = std::make_shared<coop_pow::cpp_driver> ();
		}
		else if (device.type == coop_pow_server::config::device::device_type::gpu)
		{
			driver = std::make_shared<coop_pow::opencl_driver> ();
		}
...
coop_pow_server::work_handler::~work_handler ()
{
	pool.stop ();
}

void coop_pow_server::work_handler::handle_queue_request (std::function<void(std::string)> response_handler)
{
...
void coop_pow_server::work_handler::handle_queue_delete_request (std::function<void(std::string)> response_handler)
{
	boost::property_tree::ptree response;
...
void coop_pow_server::work_handler::handle_request_async (std::string body, std::function<void(std::string)> response_handler)
{
	...
	try
	{
			...
			boost::asio::post (pool, [this, correlation_id, response_handler, create_error_response, attach_correlation_id] {
				...
				if (!jobs.empty ())
				{
					...
					try
					{
						...
						// TODO: set difficulty when handling request once the coop-pow API is set
						//       as well as set_memory based on request?
						// driver->difficulty_set(coop_pow::reverse ((1ULL << 40) - 1));
						// driver->memory_set (...);

						// TODO: coop-pow API currently takes a 64 bit integer, same with the nonce
						...
						{
							// TODO: use values from coop-pow / to_multiplier ()
							job.result.work = u128 ("2feefeefeefeefee");
						...
						else
						{
							...
							job.result.work = u128 ("2feefeefeefeefee");
							job.result.difficulty = u128 ("2feefeefeefeefee");
		...
		else if (action && *action == "work_validate")
		{
			...
			// TODO: update to use coop_pow::passes when the API is done
			bool passes (true);

			boost::property_tree::ptree response;
			response.put ("valid", passes ? "1" : "0");

			// TODO: add these when the coop_pow API is done
			response.put ("difficulty", "0x1850000000000000");
