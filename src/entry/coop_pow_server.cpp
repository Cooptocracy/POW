...
int main (int argc, char * argv[])
{
	try
	{
		...
		options.add_options () ("config_path", boost::program_options::value<std::string> ()->default_value ("./config-coop-pow-server.toml"), "Path to the optional configuration file, including the file name");
		...
		coop_pow_server::config conf (config_path_parsed.string (), config_overrides);

		// Log configuration
		auto rotating_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt> ("coop-pow-server.log", 1024 * 1024 * 5, 5);
		...
		coop_pow_server::work_handler work_handler (conf, logger);
		web::config web_conf;
		...
		logger->info ("Coop PoW Server version {}", version_string_full);
